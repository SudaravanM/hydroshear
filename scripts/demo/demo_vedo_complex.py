...
class ObjectVedo(VedoObject):
    ...
    def __init__(self, mesh_path, mtl_path=None, num_obj_pts=100):
        if mtl_path:
            self.obj_mesh: [vedo.Mesh] = vedo.file_io.load_obj(mesh_path, mtl_path)
            self.obj_mesh = [mesh.c(mesh.color() * 0.8).alpha(0.3) for mesh in self.obj_mesh]
            bounds_lower = np.min([mesh.bounds()[::2] for mesh in self.obj_mesh], axis=0)
            bounds_upper = np.max([mesh.bounds()[1::2] for mesh in self.obj_mesh], axis=0)
            self.radius = (bounds_upper - bounds_lower) / 2.0
        ...
        self.num_obj_pts = int(num_obj_pts)
...
if __name__ == '__main__':
    ...
    # object_vedo = ObjectVedo("demo_assets/sphere.obj", num_obj_pts=4e3)
    # object_vedo = ObjectVedo("demo_assets/small_cow.stl",num_obj_pts=1e4)
    object_vedo = ObjectVedo(OBJECT_PATH, num_obj_pts=4e3)
    # object_vedo = ObjectVedo("demo_assets/dumbbell_09x.stl", num_obj_pts=4e3)
    # object_vedo = ObjectVedo("demo_assets/cross2_09x.stl", num_obj_pts=4e3)
    # object_vedo = ObjectVedo("demo_assets/star_small.stl", num_obj_pts=4e3)
    object_vedo.obj_mesh = object_vedo.obj_mesh.c([119, 136, 153])
    
    # Initialize frame at object pose
    object_pose = object_vedo.get_mesh_pose_matrix()
    frame_vedo = FrameVedo(size=0.005, pose=object_pose)
    ...
    hydrosoft = HydroSoftSensor(
        ...
        lambda_d=50_000,
        lambda_s=100_000,
        dilate_scale=38461.5385 * 10,
        shear_scale=790000 / 40 / 2,
        # mu=0.5,
        # lambda_d=20000,
        # lambda_s=10800,
        # shear_scale=15384.615,
        # dilate_scale=15384.615,
        normal_axis=2
    )
    ...
    indenter_pcd = vedo.Points(r=INDENTER_PCD_SIZE, c='cyan')
    ...
    if VISUALIZE_INDENTER_PCD[0]:
        indenter_pcd = vedo.Points(indenter_pts_in_world, r=INDENTER_PCD_SIZE, c='cyan')
        viz += indenter_pcd

    step_size_container = [0.0001]
        ...
        # rotation
            ...
            if VISUALIZE_INDENTER_PCD[0]:
                ...
                if len(pts) > 0:
                    ...
                    sph = vedo.Spheres(pts, r=sphere_r / 2, c='red')
                    temp_actors.append(sph)
