...
from rl.utils.dr_utils import get_property_setter_map, get_property_getter_map, \
...
class Env(ABC):
    def __init__(self, config: Dict[str, Any], rl_device: str, sim_device: str, graphics_device_id: int, headless: bool):
        ...
        if self.use_dict_obs:
            ...
            if config["env"].get("rollout", False):
                self.obs_dims.update(config["env"].get("additional_obs", {}))
                self.add_obs_space = spaces.Dict(
                    {
                        k: spaces.Box(
                            np.ones(shape=dims) * -np.Inf, np.ones(shape=dims) * np.Inf
                        )
                        for k, dims in config["env"].get("additional_obs", {}).items()
                    }
                )
                self.obs_space = spaces.Dict(
                    {
            ...
            # self.obs_dims['eef_to_socket_pos'] = [3]
            # self.obs_dims['eef_to_socket_quat'] = [4]
            # self.obs_space['eef_to_socket_pos'] = spaces.Box(
            #     np.ones(shape=[3]) * -np.Inf, np.ones(shape=[3]) * np.Inf
            # )
            # self.obs_space['eef_to_socket_quat'] = spaces.Box(
            #     np.ones(shape=[4]) * -np.Inf, np.ones(shape=[4]) * np.Inf
            # )

            # self.obs_dims['left_tactile_camera_taxim'] = [80, 60, 3]
            # self.obs_dims['right_tactile_camera_taxim'] = [80, 60, 3]

            # self.obs_space['left_tactile_camera_taxim'] = spaces.Box(
            #     np.ones(shape=[80, 60, 3]) * -np.Inf, np.ones(shape=[80, 60, 3]) * np.Inf
            # )
            # self.obs_space['right_tactile_camera_taxim'] = spaces.Box(
            #     np.ones(shape=[80, 60, 3]) * -np.Inf, np.ones(shape=[80, 60, 3]) * np.Inf
            # )
        ...
        task_name = config.get("name", "UnknownTask")
        self.record_frames_dir = join("recorded_frames", task_name, datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
        self.record_elastomer_left_pose_dir = join(self.record_frames_dir, "elastomer_left_pose")
        self.record_elastomer_right_pose_dir = join(self.record_frames_dir, "elastomer_right_pose")
        self.record_indenter_pose_dir = join(self.record_frames_dir, "indenter_pose")
        self.record_marker_shear_left_dir = join(self.record_frames_dir, "marker_shear_left")
        self.record_marker_shear_right_dir = join(self.record_frames_dir, "marker_shear_right")
        self.record_gripper_closed_dir = join(self.record_frames_dir, "gripper_closed")

        self.record_video: bool = False
        self.video_start_step = 0
...
class VecTask(Env):
    ...
    def set_viewer(self):
        ...
        if self.headless == False:
            ...
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_B, "record_video")
    ...
    def allocate_buffers(self):
        ...
        if self.use_dict_obs:
            ...
            cprint("Obs dictionary: ", "green")
            cprint(self.obs_dims, "green")
            for k, dims in self.obs_dims.items():
                print(f'{k}: {dims}')
        ...
        self.randomize_buf = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.long)
        self.extras = {}
        self.pred_value_buf = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.float)
    ...
    def step(self, actions: torch.Tensor) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor, Dict[str, Any]]:
        ...
        self.extras["predicted_values"] = self.pred_value_buf.to(self.rl_device)
    ...
    def render(self, mode="rgb_array"):
        ...
        if self.viewer:
            ...
            for evt in self.gym.query_viewer_action_events(self.viewer):
                if evt.action == "QUIT" and evt.value > 0:
                    ...
                    if self.record_frames:
                        print("Recording frames:", self.record_frames)
                    else:
                        print("Stopped recording frames.")
                elif evt.action == "record_video" and evt.value > 0:
                    self.record_video = not self.record_video
                    if self.record_video:
                        print("Video recording: ON (press B again to stop)")
                        self.video_start_step = self.control_steps
                    else:
                        print("Video recording: OFF - Creating video...")
                        self._save_video()
            ...
            if self.record_frames or self.record_video:
    ...
    def _save_video(self):
        ...
        video_dir = join("recorded_videos", datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
        os.makedirs(video_dir, exist_ok=True)
        video_path = join(video_dir, f"video_{self.control_steps}.mp4")

        # Collect all frames from video_start_step to current step
        frames = []
        for step in range(self.video_start_step, self.control_steps + 1):
            frame_path = join(self.record_frames_dir, f"frame_{step}.png")
        ...
        # Clean up frames if only video recording was enabled
