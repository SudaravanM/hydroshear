"""
Implements Online DAgger and RMA-style Teacher-Student Distillation
"""
...
class DistillationPPO(object):
    def __init__(self, env, cfg, output_dir, teacher_ckpt=None, teacher_cfg=None):
        # Configs
        self.cfg = cfg
        ...
        self.teacher_actor_config = teacher_cfg.train.actor
        self.teacher_critic_config = teacher_cfg.train.critic

        if 'rnn' in self.teacher_actor_config:
            OmegaConf.set_struct(self.teacher_actor_config, False)
            self.teacher_actor_config['num_seqs'] = self.num_actors
            OmegaConf.set_struct(self.teacher_actor_config, True)
        if 'rnn' in self.teacher_critic_config:
            OmegaConf.set_struct(self.teacher_critic_config, False)
            self.teacher_critic_config['num_seqs'] = self.num_actors
            OmegaConf.set_struct(self.teacher_critic_config, True)
        ...
        self.teacher_model = ActorCritic(actor_obs_config=self.teacher_actor_config, critic_obs_config=self.teacher_critic_config)
        ...
        self.teacher_running_mean_std_obs = RunningMeanStdObs(
            omegaconf_to_dict(self.teacher_actor_config.input_shape),
            per_channel_obs_list=per_channel_obs
        ).to(self.device)
        self.teacher_running_mean_std_states = RunningMeanStdObs(
            omegaconf_to_dict(self.teacher_critic_config.input_shape),
            per_channel_obs_list=per_channel_states
        ).to(self.device)
        self.teacher_running_mean_std_value = RunningMeanStd((1,)).to(self.device)
        self.load_teacher(teacher_ckpt)

        self.teacher_is_rnn = True if ('rnn' in self.teacher_actor_config) or ('rnn' in self.teacher_critic_config) else False
        if self.teacher_is_rnn:
            self.teacher_actor_config['num_seqs'] = self.num_actors
            self.teacher_critic_config['num_seqs'] = self.num_actors
            teacher_actor_states = self.teacher_model.actor_network.get_default_rnn_state()
            teacher_critic_states = self.teacher_model.critic_network.get_default_rnn_state()
            teacher_actor_states = [s.to(self.device) for s in teacher_actor_states]
            teacher_critic_states = [s.to(self.device) for s in teacher_critic_states]
            self.teacher_rnn_states = [teacher_actor_states, teacher_critic_states]
            num_seqs = self.horizon_length // self.seq_length
            all_states = teacher_actor_states + teacher_critic_states
            self.mb_teacher_rnn_states = [torch.zeros((num_seqs, s.size()[0], self.num_actors, s.size()[2]), dtype=torch.float32, device=self.device) for s in all_states]
        else:
            self.teacher_rnn_states = None

        cprint(f"freeze_critic: {self.freeze_critic}", "green")

    def compute_rma_loss(self, student_features, teacher_features):
        # return F.l1_loss(student_features, teacher_features)
        return (student_features - teacher_features).pow(2).mean()

    def compute_bc_loss(self, student_actions, teacher_actions):
    ...
    def train(self):
        ...
        while self.agent_steps < self.max_agent_steps:
            self.epoch_num += 1
            a_losses, c_losses, b_losses, entropies, kls, grad_norms, mus, sigmas, bc_losses, rma_losses = self.train_epoch()
            ...
            self.write_stats(a_losses, c_losses, b_losses, entropies, kls, grad_norms, mus, sigmas, bc_losses, rma_losses)
    ...
    def train_epoch(self):
        ...
        for epoch_idx in range(0, max_mini_epochs):
            ...
            for i in range(len(self.experience_buffer)):
                ...
                if self.teacher_is_rnn:
                    teacher_rnn_states_minibatch = input_dict['teacher_rnn_states']
                    teacher_rnn_states_minibatch = [
                        (teacher_rnn_states_minibatch[0], teacher_rnn_states_minibatch[1]),
                        (teacher_rnn_states_minibatch[2], teacher_rnn_states_minibatch[3]),
                    ]
                else:
                    teacher_rnn_states_minibatch = None
                ...
                teacher_batch_dict = {
                    'prev_actions': actions,
                    'obs': None,
                    'states': self.teacher_running_mean_std_states(filtered_states) if self.normalize_input else filtered_states,
                    'rnn_states': teacher_rnn_states_minibatch,
                    # 'seq_length': self.seq_length if self.is_rnn else 1,
                }
                # res_dict = self.model(batch_dict)
                res_dict = self.model.forward_with_features(batch_dict)
                with torch.no_grad():
                    teacher_res_dict = self.teacher_model.forward_with_features(teacher_batch_dict)
                action_log_probs = res_dict['prev_neglogp']
                values = res_dict['values']  # NOTE: Already normalized if normalize_value=True
                entropy = res_dict['entropy']
                mu = res_dict['mus']
                sigma = res_dict['sigmas']
                student_features = res_dict['actor_features']
                teacher_features = teacher_res_dict['actor_features']
                student_actions = res_dict['mus']
                teacher_actions = teacher_res_dict['mus']
                ...
                rma_loss = self.compute_rma_loss(student_features, teacher_features)
                # bc_loss = self.compute_bc_loss(student_actions.detach(), teacher_actions.detach())
                bc_loss = torch.sum(torch.abs(torch.clamp(student_actions, -1.0, 1.0) - torch.clamp(teacher_actions, -1.0, 1.0)), dim=-1).mean()
                actor_loss = rma_loss * self.ppo_config.rma_loss_coef + bc_loss * self.ppo_config.bc_loss_coef

                bc_losses.append(bc_loss.detach())
                rma_losses.append(rma_loss.detach())
        ...
        return a_losses, c_losses, b_losses, entropies, kls, grad_norms, mus, sigmas, bc_losses, rma_losses
    ...
    def play_steps_rnn(self):
        ...
        mb_teacher_rnn_states = self.mb_teacher_rnn_states
        ...
        for n in range(self.horizon_length):
            ...
            if n % self.seq_length == 0:
                teacher_flat_states = []
                for states_tuple in self.teacher_rnn_states:  # teacher_actor_states, teacher_critic_states
                    for s in states_tuple:  # h, c for LSTM
                        teacher_flat_states.append(s)
                for s, mb_s in zip(teacher_flat_states, mb_teacher_rnn_states):
                    mb_s[n // self.seq_length, :, :, :] = s
            ...
            with torch.no_grad():
                teacher_res_dict = self.teacher_model_act(self.obs)
            self.teacher_rnn_states = teacher_res_dict['rnn_states']
            ...
            env_done_indices = all_done_indices[::self.num_actors]
        ...
        teacher_states = []
        for mb_s in self.mb_teacher_rnn_states:
            t_size = mb_s.size()[0] * mb_s.size()[2]
            h_size = mb_s.size()[3]
            teacher_states.append(mb_s.permute(1, 2, 0, 3).reshape(-1, t_size, h_size))
        self.experience_buffer.data_dict['teacher_rnn_states'] = teacher_states
    ...
    def teacher_model_act(self, obs_dict):
        filtered_obs_dict = {k: v for k, v in obs_dict['obs'].items() if k in self.teacher_actor_config.input_shape}
        filtered_obs_dict = _preproc_obs(filtered_obs_dict)
        filtered_states_dict = {k: v for k, v in obs_dict['states'].items() if k in self.teacher_critic_config.input_shape}
        filtered_states_dict = _preproc_obs(filtered_states_dict)
        input_dict = {
            'obs': None, # NOTE: Teacher uses states only
            'states': self.teacher_running_mean_std_states(filtered_states_dict) if self.normalize_input else filtered_states_dict,
            'rnn_states': self.teacher_rnn_states if self.teacher_is_rnn else None,
            # 'seq_length': self.seq_length if self.teacher_is_rnn else 1,
        }
        res_dict = self.teacher_model.act(input_dict)
        if self.normalize_value:
            res_dict['values'] = self.teacher_running_mean_std_value(res_dict['values'], denorm=True)
    ...
    def test(self):
        ...
        while True:
            ...
            mu, _, _ = self.model.act_inference(input_dict)
    ...
    def write_stats(self, a_losses, c_losses, b_losses, entropies, kls, grad_norms, mus, sigmas, bc_losses, rma_losses):
        ...
        self.writer.add_scalar('losses/bc_loss', torch.mean(torch.stack(bc_losses)).item(), self.agent_steps)
        self.writer.add_scalar('losses/rma_loss', torch.mean(torch.stack(rma_losses)).item(), self.agent_steps)
    ...
    def load_teacher(self, ckpt_path):
        checkpoint = torch.load(ckpt_path)
        ...
        if self.normalize_value:
            self.teacher_running_mean_std_value.load_state_dict(checkpoint['running_mean_std_value'])
            self.teacher_running_mean_std_value.eval()

        if self.teacher_model.is_rnn:
            self.teacher_model.actor_network.num_seqs = self.num_actors
            self.teacher_model.critic_network.num_seqs = self.num_actors
            teacher_actor_states = self.teacher_model.actor_network.get_default_rnn_state()
            teacher_critic_states = self.teacher_model.critic_network.get_default_rnn_state()
            teacher_actor_states = [s.to(self.device) for s in teacher_actor_states]
            teacher_critic_states = [s.to(self.device) for s in teacher_critic_states]
        ...
        cprint("Teacher model loaded successfully", "green")
