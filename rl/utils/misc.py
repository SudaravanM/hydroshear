...
def load_config_from_checkpoint(ckpt_path):
    """
    Load config from checkpoint directory. Looks for config.yaml in the same
    directory as the checkpoint (similar to how Wandb saves configs).
    """
    # Get the directory containing the checkpoint
    ckpt_dir = os.path.dirname(ckpt_path)
    # Go up one level to get the output directory (since checkpoints are usually in nn/ subdirectory)
    ...
    if os.path.exists(config_path):
        ...
        print(f"No config.yaml found at {config_path}, using default config")
        return None
